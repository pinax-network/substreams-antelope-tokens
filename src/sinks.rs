...
use substreams_database_change::pb::database::{DatabaseChanges, table_change};
...
#[substreams::handlers::map]
pub fn ch_out(map_accounts: Accounts, map_stats: Stats, map_transfers: TransferEvents) -> Result<DatabaseChanges, Error> {
    ...
    for account in map_accounts.items {
        ...
        tables
            ...
            .change("action_index", ("", account.action_index.to_string().as_str()))
    ...
    for stat in map_stats.items {
        let key = to_key(&stat.trx_id, stat.action_index);
        tables.push_change("token_events", key, 0, table_change::Operation::Create)
    ...
    for transfer in map_transfers.items {
        ...
        tables
            ...
            .change("action_index", ("", transfer.action_index.to_string().as_str()))
